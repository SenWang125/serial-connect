...
source "$HERE/serial-common.sh" 2>/dev/null || true
fails=0
ok(){ printf '  PASS  %s\n' "$1"; }
no(){ printf '  FAIL  %s\n' "$1"; fails=$((fails+1)); }

echo "1. GAP marker must return the cursor to column 0 (raw-mode terminals)"
if grep -q "f'\\\\r\\\\n<<serial-agent GAP" "$HERE/serial-agent"; then
...
if grep -q '/dev/tcp/' "$HERE/serial-connect"; then
...
if grep -q 'nc -z' "$HERE/serial-connect"; then
